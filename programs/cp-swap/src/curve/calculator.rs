...
impl CurveCalculator {
    ...
    pub fn swap_base_input(
        ...
        let input_amount_less_fees = if is_fee_on_input {
            // debit the fee to calculate the amount swapped
            trade_fee = Fees::trading_fee(input_amount, trade_fee_rate)?;
            protocol_fee = Fees::protocol_fee(trade_fee, protocol_fee_rate)?;
            fund_fee = Fees::fund_fee(trade_fee, fund_fee_rate)?;
        ...
        let output_amount = if is_fee_on_input {
            output_amount_swapped
        } else {
            trade_fee = Fees::trading_fee(output_amount_swapped, trade_fee_rate)?;
            protocol_fee = Fees::protocol_fee(trade_fee, protocol_fee_rate)?;
            fund_fee = Fees::fund_fee(trade_fee, fund_fee_rate)?;
            output_amount_swapped.checked_sub(trade_fee)?
        };
    ...
    pub fn swap_base_output(
        ...
        let actual_output_amount = if is_fee_on_input {
            output_amount
        } else {
            let out_amount_with_trade_fee =
                Fees::calculate_pre_fee_amount(output_amount, trade_fee_rate)?;
            trade_fee = out_amount_with_trade_fee - output_amount;
            protocol_fee = Fees::protocol_fee(trade_fee, protocol_fee_rate)?;
            fund_fee = Fees::fund_fee(trade_fee, fund_fee_rate)?;
            out_amount_with_trade_fee
        };

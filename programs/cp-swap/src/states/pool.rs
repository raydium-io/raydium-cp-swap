...
impl TryFrom<u8> for FeeOn {
    type Error = ErrorCode;
    fn try_from(value: u8) -> std::result::Result<Self, Self::Error> {
        match value {
            0 => Ok(FeeOn::InputToken),
            1 => Ok(FeeOn::Token0),
            2 => Ok(FeeOn::Token1),
...
impl PoolState {
    ...
    pub fn is_fee_on_input_token(&self, direction: TradeDirection) -> Result<bool> {
        let fee_on = FeeOn::try_from(self.fee_on)?;
        Ok(match (fee_on, direction) {
            (FeeOn::InputToken, _) => true,
            (FeeOn::Token0, TradeDirection::ZeroForOne) => true,
            (FeeOn::Token1, TradeDirection::OneForZero) => true,
            _ => false,
        })
    }
